...
class PermissionUtil {
    ...
    static async checkPermissions(action, options) {
        ...
        if (options.ensureTargetIsInGuild) {
            ...
            if (!found)
                return new PermissionError_1.PermissionError({ message: "**Error:** That user is not in this server!" });
        }
        ...
        if (options.checkTargetIsBelowClient) {
            ...
            if (targetMember.roles.highest.position >= me.roles.highest.position)
                return new PermissionError_1.PermissionError({ message: "**Error:** That member's roles are higher or equal to mine. I cannot perform that command on them!" });
        }
        ...
        if (options.checkTargetIsBelowIssuer) {
            ...
            if (targetMember.roles.highest.position >= issuingMember.roles.highest.position)
                return new PermissionError_1.PermissionError({ message: "**Error:** That user's roles are higher or equal to yours. You cannot perform that command on them!" });
        }
    }
}
